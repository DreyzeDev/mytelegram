...
/// <para><c>See <a href="https://corefork.telegram.org/method/auth.sendCode"/> </c></para>
/// </summary>
/// <remarks>
...
internal sealed class SendCodeHandler(ICommandBus commandBus, IPeerHelper peerHelper, IOptionsMonitor<MyTelegramMessengerServerOptions> options, IQueryProcessor queryProcessor, ICacheManager<FutureAuthTokenCacheItem> cacheManager, IHashHelper hashHelper, ICountryHelper countryHelper, ICacheHelper<long, long> cacheHelper, IScheduleAppService scheduleAppService, ILayeredService<IAuthorizationConverter> authorizationLayeredService, IUserConverterService userConverterService, IVerificationCodeGenerator verificationCodeGenerator, IEventBus eventBus) : RpcResultObjectHandler<Schema.Auth.RequestSendCode, Schema.Auth.ISentCode>
{
    ...
    private async Task<ISentCode> SendSmsCodeAsync(IUserReadModel? userReadModel, IRequestInput input, string phoneNumber, string phoneCodeHash, int timeout)
    {
        var code = verificationCodeGenerator.Generate();
    ...
    private async Task<(ISentCode? sentCode, bool loginSuccess)> SignInWithFutureAuthTokenAsync(IRequestInput input, RequestSendCode obj, IUserReadModel userReadModel)
    {
        if (obj.Settings.LogoutTokens?.Count > 0)
        {
            if (obj.Settings.LogoutTokens?.Count > 0)
            {
                var cacheKeys = obj.Settings.LogoutTokens.Take(_maxFutureAuthTokens).Select(p => FutureAuthTokenCacheItem.GetCacheKey(BitConverter.ToString(SHA1.HashData(p.Span)).Replace("-", string.Empty))).ToList();
